...
public class Main {
	...
	private static void partida(List<Ronda> rondas, String espacio, String espacioRonda,
	...
	private static List<Ronda> rondas(String Resultados) {// El problema en esta funcion era que creaba la ronda al
															// encontrarse con una linea vacia y en nuestro archivo no
															// pasaba, asi que para solucionarlo cree la ronda al leer
															// el renglon donde arranca la ronda.
		...
		try {
			for (String linea : Files.readAllLines(Paths.get(Resultados))) {
				...
				/*
				 * else if (linea.isEmpty()){ List<Partido> partidos1 = new
				 * ArrayList<Partido>(partidos); partidos.clear(); rondas.add(new
				 * Ronda(numRonda, partidos1));
				 * 
				 * }
				 */
	...
	private static List<Participante> participantes(String Pronosticos) {
		...
		try {
			for (String linea : Files.readAllLines(Paths.get(Pronosticos))) {
				...
				/*
				 * else if (linea.isEmpty()){ List<Pronostico> pronostico1 = new
				 * ArrayList<Pronostico>(pronosticos); pronosticos.clear();
	 ...
	 * 
	 * try { int i = 1; for (String linea :
	 * Files.readAllLines(Paths.get(Resultados))) { String equipo1 =
	 * linea.split(",")[0]; int goles1 = Integer.parseInt(linea.split(",")[1]);
	 * String equipo2 = linea.split(",")[3]; int goles2 =
	 * Integer.parseInt(linea.split(",")[2]); Partido partido = new Partido(equipo1,
	 * equipo2, goles1, goles2); // partidos[i - 1] = new Partido(equipo1, equipo2,
	 * goles1, goles2); ronda.partidos[i - 1] = partido;
	 * System.out.println("Datos del partido " + i + ronda.partidos[i -
	 * 1].toString()); System.out.println("Ganador del partido " + i + " = " +
	 * ronda.partidos[i - 1].equipoGanador() + "\n"); System.out.println(espacio);
	 * i++;
	 * 
	 * } } catch (IOException e) { e.printStackTrace(); }
	 * 
	 * try { int i = 1; for (String linea :
	 * Files.readAllLines(Paths.get(Pronosticos))) { String equipo1 =
	 * linea.split(",")[0]; String equipo2 = linea.split(",")[1]; String resultado =
	 * linea.split(",")[2]; Pronostico pronostico = new Pronostico(equipo1, equipo2,
	 * resultado); // pronosticos[i - 1] = new Pronostico(equipo1, equipo2,
	 * resultado); ronda.pronosticos[i - 1] = pronostico;
	 * System.out.println("Prediccion del ganador del partido " + i + " = " +
	 * ronda.pronosticos[i-1].getRersultado()); System.out.println(espacio); i++;
	 * 
	 * } } catch (IOException e) { e.printStackTrace(); }
	 * 
	 * System.out.println("\n\nEl puntaje obtenido en esta ronda es: " +
	 * ronda.calcularPuntaje() + ".");
	 * 
	 * }
	 */
