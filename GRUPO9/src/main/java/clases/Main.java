...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/grupo9", "root", "root");

			Statement stmt = con.createStatement();

			// USO DE LA DB
			ResultSet rs = stmt.executeQuery(
					"SELECT num_jugador , participante , equipo1, equipo2, resultado FROM grupo9.pronosticos;");
	...
	public static void partida(List<Ronda> rondas, String espacio, String espacioRonda,
		...
		for (int i = 0; i < rondas.size(); i++) {
			...
			for (int p = 0; p < participantes.size(); p++) {// En caso de que se hayan equivocado volvemos a setear el
															// valor de la bandera
				participantes.get(p).setRonda_perfecta(true);
			}
			for (int j = 0; j < rondas.get(i).getPartidos().size(); j++) {
				...
				if (j == ((rondas.get(i).getPartidos().size()) - 1)) {// Si es el ultimo partido de la ronda evalua si
																		// hubo
																		// errores en la ronda y asigna puntos extra.
				...
				for (int k = 0; k < participantes.size(); k++) {
					...
					if (participantes.get(k).isRonda_perfecta() & j == rondas.get(i).getPartidos().size() - 1) {
						//si es el ultimo partido de la ronda y la la ronda es perfecta felicita al jugador.
						System.out.println("\t\tRonda perfecta!");
					}
					System.out.println(espacioRonda);
	 ...
	 * List<Participante> participantes = new ArrayList<Participante>(); int
	 * indiceParticipante = -1; String nombre = "";
	 * 
	 * try { for (String linea : Files.readAllLines(Paths.get(Pronosticos))) { if
	 * (linea.split(",")[0].equalsIgnoreCase("Participante")) { nombre =
	 * linea.split(",")[1]; List<Pronostico> pronosticos1 = new
	 * ArrayList<Pronostico>(); participantes.add(new Participante(nombre,
	 * pronosticos1)); indiceParticipante++;
	 * 
	 * } else if (!linea.isEmpty()) { String equipo1 = linea.split(",")[0]; String
	 * equipo2 = linea.split(",")[1]; String resultado = linea.split(",")[2];
	 * Pronostico pronostico = new Pronostico(equipo1, equipo2, resultado);
	 ...
	 * }
	 * 
	 * } catch (IOException e) { e.printStackTrace(); } return participantes; }
	 */
}